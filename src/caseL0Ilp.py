...
try:
    ...
    level = 0
    G = caseL0IlpUtils.find_vc_edges(G, level)                              # finds and adds all VCs for specified level to G
    ...
    x = m.addMVar(shape=(len(P)+number_of_VCs), vtype=GRB.BINARY, name="x")

    # Set objective                 -> routing_fee(t, p, ch_ij)
    fee_dict, obj = caseL0IlpUtils.set_objective(P, T, G, number_of_VCs)
